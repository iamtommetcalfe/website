...
export function useAnalytics(router?: Router) {
  ...
  const trackPageView = async (path: string, title: string): Promise<void> => {
    ...
    if (import.meta.env.DEV) {
      console.log(`Page view (dev mode): ${path}, ${title}`);
      return;
    }
   ...
   */
  const trackEvent = async (eventName: string, params: Record<string, any> = {}): Promise<void> => {
    await loadAnalytics();
    
    if (import.meta.env.DEV) {
      console.log(`Event (dev mode): ${eventName}`, params);
      return;
    }
